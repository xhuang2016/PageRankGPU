...
__global__ void VertexPull(int* outdegree,int* csc_offset,int* csc_outgoing,const int N, float* currentPageRank, float* previousPageRank);
...
// exit(1);
  ...
  for(int i = 0; i<N_node+1;i++){
    csc_offset[i] = row_offset_2[i];
  }

  for(int i = 0; i<N_edge;i++){
    csc_outgoing[i] = column_index_2[i];
  }
  ...
  while(iteration_time<2000000){
    ...
    VertexPull<<<numBlocks,threadPerBlock>>>(d_outdegree,d_csc_offset,d_csc_outgoing,N,d_currentPageRank,d_previousPageRank);
  ...
  std::fstream output;
  output.open("VertexPullResult.txt",std::ios::out);
...
__global__ void VertexPull(int* outdegree,int* csc_offset,int* csc_outgoing,const int N, float* currentPageRank, float* previousPageRank){
  ...
  if(i<N){
    float pagerank_value = 0;
    // for(int j = 0;j<N;j++){
    //   pagerank_value += d/(N)*previousPageRank[j];
    // }
    pagerank_value += d/N;
    for(int j = csc_offset[i];j<csc_offset[i+1];j++){
      pagerank_value += (1-d)*previousPageRank[csc_outgoing[j]]/outdegree[csc_outgoing[j]];
    }
    currentPageRank[i] = pagerank_value;
  }
}
